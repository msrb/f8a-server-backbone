...
class Postgres:
    def __init__(self):
        self.connection = 'postgresql://{user}:{password}@{pgbouncer_host}:{pgbouncer_port}' \
            ...
            format(user=os.getenv('POSTGRESQL_USER', 'coreapi'),
                   password=os.getenv('POSTGRESQL_PASSWORD', 'coreapi123'),
                   pgbouncer_host=os.getenv('PGBOUNCER_SERVICE_HOST', 'mitpatel-bayesiandb.cqkrox9mwgno.us-east-1.rds.amazonaws.com'),
                   pgbouncer_port=os.getenv('PGBOUNCER_SERVICE_PORT', '5432'),
                   database=os.getenv('POSTGRESQL_DATABASE', 'coreapi'))
